...
export class AmountRequired {
    ...
    get description(): string {
        ...
        const actual: string = (+utils.formatEther(this._currentValue)).toFixed(
            4
        );

        const required: string = (+utils.formatEther(
            this._requiredValue
        )).toFixed(4);

        return `${this._name.padEnd(14)} | ${status.padEnd(
            14
        )} | actual: ${actual.padStart(
            12
        )} RBTC | required: ${required.padStart(12)} RBTC`;
    }
}
