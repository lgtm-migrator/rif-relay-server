...
export class RelayServer extends EventEmitter {
  ...
  async validateRequestWithVerifier(
    ...
    if (isDeployRequest) {
      ...
      const gasFromRequest = relayReq.relayRequest.request.gas as BigNumberish;
      ...
      if (
        estimatedDesinationGasCost.gt(gasFromRequestMaxAgreed.toUnsafeFloat())
      ) {
        throw new Error(
...
`);
  ...
  async getAllHubEventsSinceLastScan(): Promise<Array<Array<TypedEvent>>> {
    ...
    const events = await this.contractInteractor.getPastEventsForHub(options);
    if (events.length !== 0) {
