...
export class RelayServer extends EventEmitter {
    ...
    constructor(dependencies: ServerDependencies) {
        ...
        const blockchainConfig: BlockchainConfig =
            configuration.get('blockchain');
        // this.versionManager = new VersionsManager(VERSION);
        this.config = configureServer(
            contractsConfig,
            appConfig,
            blockchainConfig
        );
    ...
    async tokenHandler(verifier?: string): Promise<TokenResponse> {
        ...
        for (const verifier of verifiersToQuery) {
            const tokenHandlerInstance = ITokenHandler__factory.connect(
                verifier,
                this.contractInteractor.provider
            );
            const acceptedTokens =
                await tokenHandlerInstance.getAcceptedTokens();
    ...
    isDeployRequest(
        ...
        let isDeploy = false;
        if ('index' in req.relayRequest.request) {
            isDeploy = true;
        }
    ...
    validateInput(
        ...
        const relayWorker = req.relayRequest.relayData.relayWorker as string;
        // Check the relayWorker (todo: once migrated to multiple relays, check if exists)
        if (relayWorker.toLowerCase() !== this.workerAddress.toLowerCase()) {
            throw new Error(`Wrong worker address: ${relayWorker}\n`);
        }
    ...
    async validateMaxNonce(relayMaxNonce: number): Promise<void> {
        ...
        const nonce = await this.transactionManager.pollNonce(
            this.workerAddress
        );
                ...
                await this.contractInteractor.estimateDestinationContractCallGas(
                    {
                        ...
                        gasPrice:
                            relayReq.relayRequest.relayData.gasPrice.toString(),
                        data: relayReq.relayRequest.request.data.toString()
                    }
                );

            const gasFromRequest = relayReq.relayRequest.request
                .gas as BigNumberish;
            const maxEstimatedGasDeviation = FixedNumber.from(
                MAX_ESTIMATED_GAS_DEVIATION
            );
            const fixedOne = FixedNumber.from('1');
            const fixedGasFromRequest = FixedNumber.from(gasFromRequest);
            const gasFromRequestMaxAgreed = maxEstimatedGasDeviation
                .addUnsafe(fixedOne)
                .mulUnsafe(fixedGasFromRequest);

            if (
                estimatedDesinationGasCost.gt(
                    gasFromRequestMaxAgreed.toUnsafeFloat()
                )
            ) {
                throw new Error(
    ...
    async intervalHandler(): Promise<void> {
        ...
        if (!this.config.app.devMode) {
            workerTimeout = setTimeout(() => {
                ...
                log.warn(
                    chalk.bgRedBright(
                        `Relay state: Timed-out after ${timedOut}`
                    )
                );
        ...
        return new Promise<void>((resolve, reject) => {
            this.contractInteractor
                ...
                .catch((e) => {
                    ...
                    const error = e as Error;
                    log.error(
                        `error in worker: ${error.message} ${error.stack ?? ''}`
                    );
                    this.lastSuccessfulRounds = 0;
                    reject(error);
                })
    ...
    async _workerSemaphore(blockNumber: number): Promise<void> {
        if (this._workerSemaphoreOn) {
            log.warn(
                'Different worker is not finished yet, skipping this block'
            );
    ...
    _initTrustedVerifiers(verifiers: string[] = []): void {
        ...
        if (
            this.config.contracts.relayVerifierAddress !==
        ...
        if (
            this.config.contracts.deployVerifierAddress !==
                constants.AddressZero &&
            !this.trustedVerifiers.has(
...
`);
    ...
    async _handleChanges(currentBlockNumber: number): Promise<string[]> {
        ...
        const hubEventsSinceLastScan =
            await this.getAllHubEventsSinceLastScan();
    ...
    async _shouldRegisterAgain(
        ...
        const isPendingActivityTransaction =
            (await this.txStoreManager.isActionPending(
    ...
    _shouldRefreshState(currentBlock: number): boolean {
        return (
            currentBlock - this.lastRefreshBlock >=
                this.config.blockchain.refreshStateTimeoutBlocks ||
            !this.isReady()
        );
    }
        ...
        for (const event of hubEventsSinceLastScan) {
            switch (event.event) {
                case 'TransactionRejectedByRecipient':
                    log.debug(
                        'handle TransactionRejectedByRecipient event',
                        event
                    );
                    this._handleTransactionRejectedByRecipientEvent(
                        currentBlockNumber
                    );
                    break;
                case 'TransactionRelayed':
                    log.debug(
                        `handle TransactionRelayed event: ${JSON.stringify(
                            event
                        )}`
                    );
